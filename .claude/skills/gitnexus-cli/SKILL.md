...
Run from the project root. This parses all source files, builds the knowledge graph, and writes it to `.gitnexus/` while preserving committed agent guides and skills. Omit `--index-only` only for deliberate guide and skill regeneration, then review the resulting tracked changes.
...
**When to run:** Use `analyze --index-only` for a first index, after code changes, or after a stale-index warning. Full `analyze` is reserved for deliberate regeneration of agent guides and skills. In Claude Code, the PostToolUse hook reports staleness after `git commit` and `git merge`; it does not run analysis itself.
