...
test('cast artwork stays portrait-shaped in the shared shelf on desktop and a phone', async ({ page, request }) => {
  ...
  const shelf = page.locator('section').filter({ has: page.getByRole('heading', { name: 'Cast', exact: true }) })
  const portrait = shelf.getByText('Lead Actor', { exact: true }).locator('..').locator('[class*="_portrait_"]')
