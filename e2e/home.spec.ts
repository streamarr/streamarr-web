...
function movie(index: number) {
  return {
    ...
    summary: 'A group of friends travels beyond familiar places and discovers a world full of surprises.',
    runtime: 142,
    releaseDate: '2024-01-01',
    createdOn: '2026-08-26T12:00:00Z',
    genres: [{ id: 'drama', name: 'Drama' }, { id: 'adventure', name: 'Adventure' }],
    images: [],
    backdropImages: [],
...
const home: HomeQuery = {
  ...
  libraries: ['Movies', 'Family movies', 'Documentaries'].map((name, libraryIndex) => ({
    ...
    name,
    type: 'MOVIE',
    items: { edges: Array.from({ length: 18 }, (_, index) => {
...
test.beforeEach(async ({ page, request }) => {
  ...
  await page.route('**/graphql', async (route) => {
    ...
    if (operationName === 'Libraries') return route.fulfill({ json: { data: { libraries: home.libraries } } })
...
test('continue-watching cards fit the shelf and grow with the available width', async ({ page }) => {
...
test('the hero grows for its content and adapts to the viewport without clipping text', async ({ page }, testInfo) => {
  ...
  for (const viewport of [{ width: 375, height: 667 }, { width: 812, height: 375 }, { width: 1440, height: 900 }]) {
    ...
    const layout = await hero.evaluate((element) => {
      ...
      return text.map((node) => {
        ...
        return {
          contained: rect.top >= bounds.top && rect.bottom <= bounds.bottom &&
  ...
  await expect.poll(() => hero.evaluate((element) => element.clientHeight)).toBeGreaterThan(initialHeight)
})

test('recent libraries keep readable posters and size their scroll panels to the screen', async ({ page }, testInfo) => {
  const sections = page.locator('section').filter({ has: page.getByRole('heading', { name: /^Recently added in / }) })
  ...
  await page.setViewportSize({ width: 1440, height: 900 })
  await expect.poll(() => body.evaluate((element) => element.clientHeight)).toBeGreaterThan(phoneBodyHeight)
  const desktopSections = await sections.evaluateAll((elements) => elements.map((element) => {
    const bounds = element.getBoundingClientRect()
