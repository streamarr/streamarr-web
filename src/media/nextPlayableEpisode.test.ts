...
function episode(
  number: number,
  overrides: {
    watchStatus?: 'UNWATCHED' | 'IN_PROGRESS' | 'WATCHED'
  ...
  return {
    ...
    watchProgress:
      overrides.positionSeconds != null ? { positionSeconds: overrides.positionSeconds } : null,
...
describe('nextPlayableEpisode', () => {
  it('resumes an in-progress episode ahead of any earlier unwatched one', () => {
    const playable = nextPlayableEpisode([
      {
        seasonNumber: 1,
        episodes: [episode(1), episode(2, { watchStatus: 'IN_PROGRESS', positionSeconds: 600 })],
      },
    ])
  ...
  it('starts over from the first episode when everything is watched', () => {
    const playable = nextPlayableEpisode([
      {
        seasonNumber: 1,
        episodes: [episode(1, { watchStatus: 'WATCHED' }), episode(2, { watchStatus: 'WATCHED' })],
      },
    ])
    expect(playable).toMatchObject({ id: 'e1', verb: 'Play' })
  })
